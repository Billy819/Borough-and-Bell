...
var HOURS_PER_BOROUGH = 2;
var TOTAL_HOURS = TOTAL_BOROUGHS * HOURS_PER_BOROUGH; // 66
...
function paintMap() {
  $$('#londonMap .boro').forEach(function (p) {
    ...
    p.setAttribute('aria-label', b.name + (b.order <= S.unlocked
      ? ', restored' : ', locked until ' + b.hours + ' hours'));
  });
}
function byId(id) {
  ...
  if (b.order > S.unlocked) {
    det.innerHTML =
      ...
      '<div class="locked-note">' +
        ...
        '<p>' + esc(b.name) + ' joins the map at <b>' + b.hours + ' hours</b> of focused work.<br>' +
...
function plaqueHTML(b, forceOpen) {
  ...
  if (!open) {
    return '<div class="plaque is-locked"><span class="pl-no">No. ' + b.order + '</span>' +
           ...
           '<span class="pl-hours">' + b.hours + ' hours</span></div>';
  }
...
function paintPlaques() {
  ...
  $('#plaqueCount').textContent = S.unlocked + ' of ' + TOTAL_BOROUGHS +
    ' restored · next at ' + (S.unlocked < TOTAL_BOROUGHS
      ? window.BOROUGHS[S.unlocked].hours + ' hours' : 'complete');
}
...
function wireSettings() {
  ...
  $('#btnWipe').addEventListener('click', function () {
    if (!confirm('Clear all 66 hours of progress, every borough and every task?\n\nThis cannot be undone.')) return;
